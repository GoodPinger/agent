...
/// space is an unambiguous delimiter. Unix-only (only the Unix `check_uds_http`
/// parses a target; the non-Unix stub does not).
#[cfg(unix)]
...
/// Unix-only (used only by the Unix `check_uds_http`).
#[cfg(unix)]
