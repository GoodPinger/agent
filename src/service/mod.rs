...
//! boot. `unit` is the pure, cross-platform renderer; the privileged shell is
//! Unix-only (systemd/launchd) and lives in `imp`. On non-Unix, `cmd_service` is a
//! stub that reports the feature is unsupported.

// The unit/plist renderer is only consumed by the Unix shell (`imp`); gating it
// keeps its items from being dead code on non-Unix, where only the stub compiles.
#[cfg(unix)]
pub mod unit;

/// The `gpr service` sub-actions.
// On non-Unix the stub `cmd_service` ignores the action, so the `user` fields go
// unread there — that's expected for a Unix-only feature, not a real dead field.
...
#[cfg(not(unix))]
pub fn cmd_service(_action: ServiceCmd) -> i32 {
    eprintln!(
...
mod imp {
    ...
    use super::unit::{
        launchd_path, render_launchd, render_systemd, systemd_path, Scope, UnitParams,
