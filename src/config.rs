...
pub struct Config {
    ...
    /// Fleet to self-join by name, sent additively on `/agent/hello`. Absent means
    /// ungrouped (or dashboard-assigned). Autoscaled nodes share a name to cluster.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Append a watched process, enforcing dedup by name and the `MAX_PROCESSES` cap.
pub fn add_process(cfg: &mut Config, spec: ProcessSpec) -> AddResult {
    if cfg.processes.len() >= MAX_PROCESSES {
        return AddResult::Full;
    }
    if cfg.processes.iter().any(|p| p.name == spec.name) {
        return AddResult::Duplicate;
    }
    cfg.processes.push(spec);
    AddResult::Added
}

/// Remove the watched process with the given name. Returns whether one was removed.
pub fn remove_process(cfg: &mut Config, name: &str) -> bool {
    let before = cfg.processes.len();
    cfg.processes.retain(|p| p.name != name);
    cfg.processes.len() != before
}
...
mod tests {
    ...
    #[test]
    fn add_process_dedups_and_caps() {
        let mut cfg = Config::default();
        let spec = |n: &str| ProcessSpec {
            name: n.to_string(),
            pattern: None,
        };

        assert!(matches!(
            add_process(&mut cfg, spec("nginx")),
            AddResult::Added
        ));
        assert!(matches!(
            add_process(&mut cfg, spec("nginx")),
        ...
        while cfg.processes.len() < MAX_PROCESSES {
            let n = format!("p{}", cfg.processes.len());
            assert!(matches!(add_process(&mut cfg, spec(&n)), AddResult::Added));
        }
        assert!(matches!(
            add_process(&mut cfg, spec("over")),
            AddResult::Full
        ));
    }

    #[test]
    fn remove_process_removes_by_name() {
        let mut cfg = Config {
            processes: vec![
                ProcessSpec {
                    name: "nginx".into(),
        ...
        assert!(remove_process(&mut cfg, "nginx"));
        assert_eq!(cfg.processes.len(), 1);
        assert_eq!(cfg.processes[0].name, "pg");
        assert!(!remove_process(&mut cfg, "missing"));
    }
