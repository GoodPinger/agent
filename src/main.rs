...
enum Command {
    ...
    Login {
        ...
        #[arg(long)]
        token: Option<String>,
        /// Fleet to join by name (self-joins on the next report; autoscaled nodes
        /// sharing a name cluster into one fleet).
        #[arg(long)]
        group: Option<String>,
    },
    ...
    Watch {
        ...
        #[arg(long)]
        once: bool,
        /// Run the daemon detached in the background and return the shell. Good for
        /// a quick run; for a supervised, start-on-boot setup use `gpr service install`.
        #[arg(long, short = 'b', conflicts_with = "once")]
        background: bool,
        /// Stop a background `gpr watch` started with --background.
        #[arg(long, conflicts_with_all = ["once", "background"])]
        stop: bool,
        #[command(subcommand)]
    ...
    /// Manage the auto-start service that runs `gpr watch` on boot.
    Service {
        #[command(subcommand)]
        action: ServiceAction,
    },
    /// Update gpr to the latest release (verifies the checksum, then swaps in).
    Update,
}

#[derive(Subcommand)]
enum ServiceAction {
    /// Install + enable the service (system scope needs sudo; --user for rootless).
    Install {
        #[arg(long)]
        user: bool,
    },
...
enum CheckTarget {
    ...
    Pidfile { path: String },
    /// Connect to a unix socket file (e.g. a WSGI app's socket).
    Uds { path: String },
    /// HTTP GET over a unix socket; check the response is 2xx/3xx.
    UdsHttp {
...
enum WatchAction {
    ...
    /// Interactively browse processes and manage the watchlist (full-screen UI).
    #[command(alias = "edit")]
    Manage,
}
...
enum WatchAdd {
    ...
    /// Unix socket, e.g. gpr watch add uds /run/gunicorn.sock
    Uds { target: String },
    /// HTTP over a unix socket, e.g. gpr watch add uds-http /run/gunicorn.sock /health
    UdsHttp {
        /// Path to the socket file.
        socket: String,
        /// Request path (default "/").
...
fn main() {
    ...
    let code: i32 = match cli.command {
        ...
        Command::Login { token, group } => cmd_login(token, group),
        ...
        Command::Watch {
            ...
            None if stop => watch::cmd_watch_stop(),
            None if background => watch::cmd_watch_background(),
            ...
            Some(WatchAction::Manage) => watch_ui::cmd_watch_manage(),
        },
        Command::Check { target } => match target {
            ...
            CheckTarget::Uds { path } => check::cmd_uds(&path),
            CheckTarget::UdsHttp { socket, path } => {
                check::cmd_uds_http(&socket, path.as_deref().unwrap_or("/"))
            }
        },
        Command::Service { action } => match action {
            ServiceAction::Install { user } => {
                service::cmd_service(service::ServiceCmd::Install { user })
            }
            ServiceAction::Uninstall { user } => {
                service::cmd_service(service::ServiceCmd::Uninstall { user })
            }
            ServiceAction::Status => service::cmd_service(service::ServiceCmd::Status),
        },
        Command::Update => update::cmd_update(),
    };
...
fn cmd_login(token: Option<String>, group: Option<String>) -> i32 {
    ...
    // --group sets the fleet to self-join; a non-empty value overrides, an empty
    // one clears it. Omitting the flag leaves any existing fleet untouched.
    if let Some(g) = group {
        let g = g.trim().to_string();
        cfg.group = if g.is_empty() { None } else { Some(g) };
    }
    ...
    match Client::new(
        ...
        Ok(client) => {
            let hello = transport::HelloReq::for_host(cfg.host_id.clone(), cfg.group.clone());
...
fn cmd_watch_add(what: WatchAdd) -> i32 {
    ...
    let (kind, target) = match what {
        ...
        WatchAdd::Uds { target } => (check::KIND_UDS, target),
        WatchAdd::UdsHttp { socket, path } => (
            check::KIND_UDS_HTTP,
            check::join_uds_http_target(&socket, path.as_deref().unwrap_or("/")),
        ),
        WatchAdd::Process { name, pattern } => {
            match config::add_process(
                &mut cfg,
                config::ProcessSpec {
                    name: name.clone(),
                    pattern,
