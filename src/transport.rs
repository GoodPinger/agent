...
#[derive(Debug, Clone, Serialize)]
pub struct HelloReq {
    /// Identity UUID (never shown). See protocol §hello / §Field conventions.
    pub host: String,
    /// Machine display name for the console. Additive; omitted when the platform
    /// reports no name, in which case the server keeps the operator-given name.
    ...
    pub os: String,
    pub arch: String,
    /// Fleet to self-join by name (additive, protocol v1). Omitted when unset.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub group: Option<String>,
}

impl HelloReq {
    /// Build a hello for this host from the running platform. `group` is the
    /// configured fleet name (or None).
    pub fn for_host(host: String, group: Option<String>) -> Self {
        Self {
            host,
            hostname: sysinfo::System::host_name().filter(|h| !h.is_empty()),
