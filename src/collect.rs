...
pub struct Collector {
    ...
    /// Process names to sample, from config (`processes`).
...
impl Collector {
    ...
    pub fn sample_processes(&mut self) -> Vec<ProcessReport> {
        ...
        // Snapshot (name, pid) so we do not hold a borrow of `self.sys` across the
        // mutable `tracker.observe` call.
        let mut out = Vec::with_capacity(self.process_names.len());
        let names = self.process_names.clone();
        for name in &names {
            let pid = self.find_pid(name);
            let restarts = self.tracker.observe(name, pid, now_ms);
    ...
    /// Find the PID of the running process matching `name`. Matches by exact
    /// process name first, then by substring (Linux truncates `comm` to 15
    /// chars, so a substring match catches long names). Returns the lowest PID
    /// among matches for stability across ticks.
    fn find_pid(&self, name: &str) -> Option<u32> {
        let mut best: Option<u32> = None;
        for (pid, proc_) in self.sys.processes() {
            let pname = proc_.name().to_string_lossy();
            if pname == name || pname.contains(name) {
                let p = pid.as_u32();
...
mod tests {
    ...
    fn sample_processes_reports_configured_names() {
        ...
        let mut c = Collector::new(vec!["definitely-not-a-real-process-xyz".to_string()]);
    ...
    fn rss_helper_after_collect_is_under_ceiling() {
        ...
        let mut c = Collector::new(vec!["init".to_string()]);
        let _ = c.metrics();
