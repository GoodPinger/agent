//! `gpr watch edit` — the interactive watch manager. `state` is the pure state
...
/// `gpr watch edit` entry point.
pub fn cmd_watch_edit() -> i32 {
    if !std::io::stdin().is_terminal() || !std::io::stdout().is_terminal() {
        eprintln!(
            "{}: `{} watch edit` needs an interactive terminal — use `{} watch add`, `list`, or `rm`",
            brand::CLI,
            brand::CLI,
            brand::CLI
        );
    ...
    match result {
        Ok(code) => code,
        Err(_) => {
            eprintln!("{}: watch edit exited abnormally", brand::CLI);
            1
        }
    }
}
...
fn render(st: &ManagerState) -> std::io::Result<()> {
    ...
    // Title bar.
    let title = format!(
        " gpr watch edit — {} watched · {} processes ",
        watched.len(),
        snap.len()
    );
    queue!(
...
fn render_help(out: &mut impl Write, width: usize, rows: u16) -> std::io::Result<()> {
    queue!(
        out,
        style::PrintStyledContent(pad(" gpr watch edit — help ", width).black().on_yellow()),
        cursor::MoveToNextLine(2)
    )?;
    let keys = [
